...
const PostRequirement = () => {
  ...
  const validate = (fieldValues = input) => {
    ...
    if ("requirementDetails" in fieldValues)
      temp.requirementDetails = fieldValues.requirementDetails
        ? ""
    ...
    if ("phonenumber" in fieldValues)
      temp.phonenumber =
        fieldValues.phonenumber.length > 9 ? "" : "The length should be 10";
    ...
    if (validate()) {
      ...
      try {
        ...
        var a = await axios.post(
          "http://localhost:9000/postrequirement/addPostRequirement",
          values,
          { headers }
        );
  ...
  return (
    <div>
      ...
      <Form
        ...
        autoComplete="off"
        className="mx-auto p-5 m-5 border border-primary"
        style={{ width: "30rem" }}
      >
        <Input
          id="category"
